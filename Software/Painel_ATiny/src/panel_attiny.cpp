...
#define Number_pixel 20 //numero de leds por painel
#define Number_painel 10 //numero de paineis
#define Data_in    1 //define o pino de entrada do pixel eu acho
#define ADDR   0x01
...
Adafruit_NeoPixel pixels = Adafruit_NeoPixel(Number_pixel, Data_in, NEO_GRB + NEO_KHZ800);


void setPanelColor(int8_t r, int8_t g, int8_t b) {
  for (int8_t i = 0; i < Number_pixel; i++) {
...
void notifyLeft() {//da um sinal para a nanoleaf da esquerda
  pinMode(left, OUTPUT);
...
void notifyRight() { //da um sinal para a nanoleaf da direita
  ...
  if (howMany == 2)
  {
    ...
    if (i2cBuffer[0] == 252) {
      if (i2cBuffer[1] == 250) {
        ...
        Wire.onReceive(receiveEvent);//receiveEvent
      }
    }
  }
...
void setup()
{
  ...
  if (EEPROM.read(0x00) == 1) {
    ...
    EEPROM.write(0x00, 0);
    applyColor(0, 50, 0);
    //determina apartir da ultima posição de memoria qual o pino de entrada
...
void loop()
{
  ...
  if (!configured) {
    ...
    //grava na EEPROM o pino da entrada
